...
#define RXD2 16
#define TXD2 17
#define SEND_INTERVAL 20000 // Intervalo de Envio a TS

const char* wifi_ssid = "el_nombre_de_tu_red";
const char* wifi_pass = "la_contraseña_de_tu_red";
const char* ap_ssid   = "MonitorEnergia_ESP32"; //Nombre de la red del ESP32
const char* ap_pass   = "12345678"; //Contraseña de la red del ESP32
const char* apiKey    = "apiKey"; //Tus apiKeys
...
void readPZEM() {
  data.v = pzem.voltage(); //Voltaje
  data.c = pzem.current(); // Corriente
  data.p = pzem.power(); //Potencia
  data.e = pzem.energy(); //Energía
  data.f = pzem.frequency(); //Frecuencia
  data.fp = pzem.pf(); //Factor de Potencia
